  ...
  return (
    <div className="fixed inset-0 bg-black z-[200] p-8 flex flex-col justify-center font-mono">
      <div className="max-w-md mx-auto w-full">
        {logs.map((log, i) => (
          <div key={i} className="mb-2 text-green-500 animate-pulse">{log}</div>
        ))}
        <div className="w-full bg-green-950 h-1 mt-4">
          <div className="bg-green-500 h-full animate-load" style={{ width: `${(logs.length / sequence.length) * 100}%` }}></div>
        </div>
      </div>
    </div>
  );
